...
Use --list to see all currently scheduled commits.`,
			...
			if listFlag {
				...
				for _, repo := range resp.Repos {
					if !repo.ScheduledAt.IsZero() {
						fmt.Printf("%-50s | %-20s | %s\n", core.TruncateRepoPath(repo.Path, 50), repo.ScheduledAt.Local().Format("15:04:05"), repo.ScheduledMsg)
						found = true
					}
				}
	...
	cmd.Flags().BoolVarP(&listFlag, "list", "l", false, "List all scheduled commits")
	return cmd
}
